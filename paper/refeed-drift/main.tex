...
(problem-clustered $t = 2.9$--$6.4$). Most changes are zero-boundary
crossings of the quantized estimator rather than polarity reversals, and the
perturbation is mean-zero, so averaged quantities are largely safe; but
...
bitwise identical, with both disagreement rates exactly zero. Even the floor
...
rule; cache hits underlying every ``exact'' claim are instrumented (100\% hit
rate, 3{,}502 pivots); total compute was under \$10. We recommend that
...
relative policy optimization \citep{shao2024deepseekmath} and its descendants
...
\begin{enumerate}
  ...
  state (cache hits instrumented per request: 100\% verified across 3{,}502
  pivots), (ii) an identical second exact pass, and (iii) a re-feed pass.
  ...
  decision tokens, re-feeding changes the credit estimate 14--28pp above the
  ...
  mean-zero ($t = -1.2$), so aggregated estimates are largely unaffected.
  \item \textbf{A downstream consequence that survives the decomposition.}
  ...
  implementing \citealp{he2025nondeterminism}) makes all three passes
  bitwise identical: both disagreement rates collapse to exactly zero and
  100\% of probes are bit-exact. The effect is batch-variant kernel
  numerics, in its entirety, and the published fix eliminates it.
...
\citet{lin2024critical} identify critical tokens by contrastive rollouts
from token positions; critical-token fine-tuning \citep{ruan2025cft} forces
...
own contribution. Separately, tree-structured rollout methods
...
LLM inference nondeterminism and fix it with batch-invariant kernels; SGLang
...
\citet{zhong2026tim} diagnose training-inference mismatch (rollout engine
...
These lines concern the serving endpoint and the training loop. Our victim
...
credit methods target), restricted to positions within 2 tokens past a
16-token block boundary (block size read from the engine and asserted), at
...
\begin{description}
  ...
  3{,}502 pivots verified on both exact passes.
...
conditionings: the pre-fixed metric (any pass nonzero; shared denominator)
...
(one side zero) and strict polarity reversals. Each pass also issues one
...
(versus 3--6\% for re-feed), which validates self-consistency, and the
...
added after a smoke run. The registration is published in the repository
with an honest provenance note: it is self-attested, without an external
timestamp. Two experimental campaigns are reported. The first campaign's
...
checkpoint from the Qwen2.5-7B base; and Phi-4-mini-instruct (a distinct
...
\midrule
Main pooled (t=.7, K4) & Qwen2.5-7B-Inst & 636 & 33.8\% (3.9) & 15.1\% (1.1) & $+18.7$ & 5.51 & 8.61 \\
...
Greedy (t=0, K1)$^{\diamond}$ & Qwen2.5-7B-Inst & 189 & 40.2\% (0.5) & 13.8\% (0.0) & $+26.5$ & 3.64 & 5.66 \\
GRPO ckpt (t=.7, K4)$^{\diamond}$ & SimpleRL-Zoo-7B & 373 & 35.4\% (3.8) & 9.1\% (1.1) & $+26.3$ & 5.50 & 8.66 \\
Phi-4-mini (t=.7, K4)  & Phi-4-mini      & 370 & 48.6\% (6.2) & 20.8\% (3.0) & $+27.8$ & 3.47 & 7.79 \\
\midrule
...
(percent). Excess in percentage points. $t_{\text{clust}}$ is a
...
$K$ trend: $K{=}8$ is the strongest configuration.
...
the quantized estimator crossing zero. The perturbation is mean-zero
(pooled bias $-0.011$, $t=-1.23$), so quantities that average many pivots
...
pivots with $|\At| \geq 0.5$), the set selected under re-feed overlaps the
...
Under batch-invariant kernels both overlaps are exactly 1.0. A third to
...
configurations, while re-feeding does so at 3--6\%; pooled 90th-percentile
deltas are $3.7\times10^{-3}$ (replica) versus $4.8\times10^{-2}$
(re-feed). Probe sequences diverge within 32 greedy tokens at 22.8\% of
...
logprob delta against exact pass 1, over the four instrumented
second-campaign runs. The replica's mass concentrates at exactly zero;
re-feed's essentially never does. Under batch-invariant kernels both
...
three passes should be bitwise identical. We reran the harness with
\texttt{VLLM\_BATCH\_INVARIANT=1} on the same engine, model, and data (10
problems, 395 pivots, 106 eligible). The prediction held exactly: zero
disagreements in both comparisons, 100\% bit-exact probes in both
comparisons, and critical-token Jaccard 1.0. This simultaneously closes the
...
2--15\% of greedy probes. Prior work measured this hazard at the benchmark
...
perturbation is mean-zero, and methods that average over many tokens and
...
instrumentation (marked in Table~\ref{tab:main}). The batch-invariant
