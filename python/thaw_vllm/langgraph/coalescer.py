...
BatchSingleCallable = Callable[
    [Sequence[Sequence[Any]], Any],
    Awaitable[list[str]],
]
"""(messages_list, sampling_params) → one generation per input. Used when N>1
pending calls fall through the fork threshold. Invoking a single batched call
lets vLLM's continuous batching schedule them together and avoids the thread-
unsafe-generate deadlock that N concurrent single_callable threads would hit."""
...
class ForkCoalescer:
    ...
    def __init__(
        ...
        *,
        ...
        batch_single_callable: Optional[BatchSingleCallable] = None,
        window_ms: float = 2.0,
        ...
        self._batch_single_callable = batch_single_callable
    ...
    async def _dispatch(self, pending: list[_PendingCall]) -> None:
        ...
        if lcp_len == 0:
            await self._run_batch_or_gather(pending)
            return
    ...
    async def _run_batch_or_gather(self, pending: list[_PendingCall]) -> None:
        # Prefer one batched call so the underlying engine's continuous batcher
        # scheduler sees N prompts at once (mirroring how a real server handles
        # N concurrent HTTP requests). Fan-out via gather(*run_single) would
        # call the single_callable N times concurrently — on vLLM V1 that
        # means N threads into `LLM.generate`, which deadlocks.
        if self._batch_single_callable is not None:
            messages_list = [p.messages for p in pending]
            sampling_params = pending[0].sampling_params
            try:
                results = await self._batch_single_callable(
                    messages_list, sampling_params
                )
            except Exception as exc:
                for call in pending:
                    if not call.future.done():
            ...
            if len(results) != len(pending):
                exc = RuntimeError(
                    f"batch_single_callable returned {len(results)} results "
                    f"for {len(pending)} calls"
                )
                for call in pending:
                    if not call.future.done():
                        call.future.set_exception(exc)
