...
def main():
    ...
    ap.add_argument("--skip-fork", action="store_true",
                    help="Skip Mode B (fork+subprocess). Needed on <80 GB GPUs.")
    ap.add_argument("--trunk-tokens", type=int, default=None,
                    help="Pad trunk with realistic prior reasoning to N tokens. "
                         "None = use the natural ~200-token trunk.")
    ap.add_argument("--max-model-len", type=int, default=None,
                    help="Override vLLM max_model_len (needed when --trunk-tokens is large).")
    ap.add_argument("--gpu-memory-utilization", type=float, default=0.90,
                    help="Parent engine GPU memory fraction. Lower to 0.25 on 80 GB "
                         "when running multi-worker fork (parent + N workers must fit).")
    ...
    elif args.trunk_tokens is not None:
        # Auto-bump: trunk + rollout max_tokens + slack.
        llm_kwargs["max_model_len"] = max(
            8192, args.trunk_tokens + args.max_tokens + 512,
        )
    ...
    # Optional: pad trunk with realistic prior reasoning filler to inflate
    # prefill cost — this is where fork's savings become undeniable.
    if args.trunk_tokens is not None:
        from _trunk_padding import pad_trunk
        tok = llm.get_tokenizer()
        full_trunk, trunk_token_count = pad_trunk(
            tok, full_trunk, args.trunk_tokens, kind="reasoning",
        )
        print(f"Trunk padded to {trunk_token_count} tokens "
              f"(target {args.trunk_tokens}).")
    else:
        tok = llm.get_tokenizer()
        trunk_token_count = len(tok.encode(full_trunk, add_special_tokens=False))
    ...
    if args.skip_fork:
        print_banner("[3/4] Mode B: skipped (--skip-fork)", char="─")
        results_b = []
        elapsed_b = fork_s = workers_s = 0.0
        summary_b = {"mean": None, "max": None, "best_idx": None, "n": 0}
    else:
    ...
    if cold and not args.skip_fork:
    ...
    if args.json_out:
        ...
        samples = []
        for mode_name, results in (("A_native", results_a[:3]), ("B_fork", results_b[:3])):
            for r in results:
                score = r["score"]
                samples.append({
                    "mode": mode_name,
                    "rollout": r["rollout"],
                    "reward": score["reward"],
                    "extracted_answer": score.get("extracted_answer"),
                    "tokens": score["approx_tokens"],
                    "text_preview": r["text"],
                })
        modes = {
            "A_native_batch": {
                "elapsed_s": round(elapsed_a, 3),
                "count": len(results_a),
                "mean_reward": summary_a["mean"],
                "max_reward": summary_a["max"],
            },
            "B_fork_subprocess": {
                "ran": not args.skip_fork,
                "elapsed_s": round(elapsed_b, 3),
                "fork_s": round(fork_s, 3),
                "workers_s": round(workers_s, 3),
                "count": len(results_b),
                "mean_reward": summary_b["mean"],
                "max_reward": summary_b["max"],
            },
            "C_cold_baseline": cold,
        }
        receipt = build_receipt(
            demo="rl_rollout_simulator",
            model=args.model,
            trunk_tokens=trunk_token_count,
            timings={"load_s": load_s, "trunk_s": trunk_s},
            modes=modes,
            checks={
                "all_nonempty_a": all(len((r.get("text") or "")) > 0 for r in results_a),
                "mode_b_skipped": args.skip_fork,
            },
            samples=samples,
            extra={
                ...
                "max_model_len": llm_kwargs.get("max_model_len"),
                "gpu_memory_utilization": args.gpu_memory_utilization,
                "worker_gpu_memory": os.environ.get("THAW_WORKER_GPU_MEMORY"),
                "worker_max_model_len": os.environ.get("THAW_WORKER_MAX_MODEL_LEN"),
            },
        )
