...
/**
 * SKY-AND-GROUND ENV — the photographer's classic chrome lighting setup.
 *
 * The env map is split top/bottom by a sharp horizon. The CHROME LOOK
 * comes from this sharp transition: reflections on the top half of any
 * curved surface show the bright sky; reflections on the bottom half
 * show the dark ground; the horizon line appears as a crisp boundary
 * crossing every surface. That contrast IS what makes chrome look like
 * chrome, in real life and in renders.
 *
 *   • SKY (top half): bright white-to-lavender gradient, plus a hot
 *     "sun spot" in the upper area for sharp specular highlights
 *   • HORIZON: sharp dark/light transition at the middle
 *   • GROUND (bottom half): deep navy-to-near-black gradient, with a
 *     subtle bluish "floor bounce" right below the snowflake position
 */
function buildSkyGroundEnvTexture(): THREE.Texture {
  const W = 1024;
  const H = 512;
  const canvas = document.createElement("canvas");
  canvas.width = W;
  ...
  // Continuous gradient top→bottom. Cobalt-tinted but with brighter
  // top values so chrome reflections show clear contrast. The holographic
  // shader emission provides the color drama; this env provides the
  // chrome-mirror grounding so the surface still reads as metal.
  const grad = ctx.createLinearGradient(0, 0, 0, H);
  grad.addColorStop(0.00, "#f0f4ff"); // bright cool white at zenith
  grad.addColorStop(0.18, "#c4d4f8"); // pale cobalt
  grad.addColorStop(0.36, "#6e8ad8"); // saturated cobalt
  grad.addColorStop(0.46, "#3a4d96"); // deep cobalt (upper haze)
  grad.addColorStop(0.54, "#243368"); // navy (mid haze)
  grad.addColorStop(0.62, "#16204a"); // deep navy (lower haze)
  grad.addColorStop(0.78, "#0a0e26");
  grad.addColorStop(0.92, "#03050f");
  grad.addColorStop(1.00, "#000000");
  ctx.fillStyle = grad;
  ctx.fillRect(0, 0, W, H);

  // SUN SPOT — bright cool-white hot center. Pushed back up to near-
  // pure white at the core so the bloom pass on the Canvas has a
  // saturated highlight to grab and turn into a real glow halo.
  // Still cool-tinted around the edges so it stays in the cobalt
  // palette overall.
  const sun = ctx.createRadialGradient(W / 2, H * 0.20, 0, W / 2, H * 0.20, 140);
  sun.addColorStop(0.0, "rgba(255, 255, 255, 1.0)");
  sun.addColorStop(0.25, "rgba(230, 240, 255, 0.75)");
  sun.addColorStop(0.55, "rgba(160, 190, 245, 0.30)");
  sun.addColorStop(1.0, "rgba(160, 190, 245, 0)");
  ctx.fillStyle = sun;
  ctx.fillRect(0, 0, W, H);

  // FLOOR BOUNCE — a soft cool light directly under the camera position.
  // This is the "from below" light the user asked for; on chrome it
  // shows as a wraparound illumination on the bottom curves of the
  // snowflake, keeping the lower half from going pure silhouette.
  const floor = ctx.createRadialGradient(W / 2, H * 0.84, 0, W / 2, H * 0.84, 200);
  floor.addColorStop(0.0, "rgba(180, 200, 255, 0.85)");
  floor.addColorStop(0.5, "rgba(120, 150, 220, 0.35)");
  floor.addColorStop(1.0, "rgba(180, 200, 255, 0)");
  ctx.fillStyle = floor;
  ctx.fillRect(0, 0, W, H);

  // Side accents — also cobalt-blue (was purple). Subtle, just adds
  // some lateral lift so the side curves don't go pure black.
  const leftAccent = ctx.createRadialGradient(120, H * 0.5, 0, 120, H * 0.5, 280);
  leftAccent.addColorStop(0.0, "rgba(110, 145, 230, 0.28)");
  leftAccent.addColorStop(1.0, "rgba(110, 145, 230, 0)");
  ctx.fillStyle = leftAccent;
  ctx.fillRect(0, 0, W, H);

  const rightAccent = ctx.createRadialGradient(W - 120, H * 0.5, 0, W - 120, H * 0.5, 280);
  rightAccent.addColorStop(0.0, "rgba(140, 165, 240, 0.28)");
  rightAccent.addColorStop(1.0, "rgba(140, 165, 240, 0)");
...
function SkyGroundEnvironment() {
  const { gl, scene } = useThree();
  useEffect(() => {
    const pmrem = new THREE.PMREMGenerator(gl);
    pmrem.compileEquirectangularShader();
    const sourceTex = buildSkyGroundEnvTexture();
